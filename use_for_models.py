...
def train_and_validation_inputs_targets(pdTrain, cv_folds = 5, randomseed = 1):
    np.random.seed(randomseed) # Use a random seed = 0 to ensure reproducability when creating cross validation folds

    inputs, targets = inputs_and_targets(pdTrain)
    
    folds = create_cv_folds(len(pdTrain), cv_folds) # Get n (= cv_folds) train and test filters from create_cv_folds to use cross validation
